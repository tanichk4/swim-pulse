...
export default function OnboardingScreen({ initialProfile }: Props) {
  ...
  const [sex, setSex] = useState<"male" | "female">(
    initialProfile?.sex ?? "male",
  );
  const [goal, setGoal] = useState(
    (initialProfile?.daily_goal ?? 500).toString(),
  );
  ...
  function checkWarnings(h: string, w: string, a: string) {
    ...
    if (!isNaN(av) && !isNaN(hv) && hv > 0) {
      if (av < 7 && hv > 130)
        warnings.height = `Height above 130 cm is unusual for age ${av}`;
      else if (av < 10 && hv > 155)
        warnings.height = `Height above 155 cm is unusual for age ${av}`;
      else if (av < 14 && hv > 180)
        warnings.height = `Height above 180 cm is unusual for age ${av}`;
    }

    if (!isNaN(av) && !isNaN(wv) && wv > 0) {
      if (av < 7 && wv > 35)
        warnings.weight = `Weight above 35 kg is unusual for age ${av}`;
      else if (av < 10 && wv > 50)
        warnings.weight = `Weight above 50 kg is unusual for age ${av}`;
      else if (av < 14 && wv > 70)
        warnings.weight = `Weight above 70 kg is unusual for age ${av}`;
    }
    ...
    if (!isNaN(hv) && !isNaN(wv) && hv > 0 && wv > 0) {
      ...
      if (bmi < 12)
        warnings.bmi = `BMI of ${bmi.toFixed(1)} seems very low — double-check values`;
      else if (bmi > 60)
        warnings.bmi = `BMI of ${bmi.toFixed(1)} seems very high — double-check values`;
    }
  ...
  async function handleSubmit() {
    ...
    if (isNaN(h)) {
      ...
      toastErrors.push(
        `<strong>${h} cm</strong> doesn't look right — expected 50–250 cm`,
      );
    }
    ...
    if (isNaN(w)) {
      ...
      toastErrors.push(
        `<strong>${w} kg</strong> doesn't look right — expected 10–300 kg`,
      );
    }
    ...
    if (isNaN(a)) {
      ...
      toastErrors.push(
        `<strong>${a} years</strong> doesn't seem right — expected 1–120`,
      );
    }
    ...
    if (isNaN(g)) {
      ...
      toastErrors.push(
        `<strong>${g} kcal</strong> doesn't look right — expected 50–5000 kcal`,
      );
    }
      ...
      <div id="screenOnboarding" className="screen active">
        <div className="onboarding-card">
          ...
          <div className="onboarding-why">
            <p>
              Your height, weight, and age let us calculate accurate calorie
              burn for every swim — no guesswork.
            </p>
          </div>
          ...
          <div className="onboarding-fields">
            ...
            <div className="onboarding-field">
              ...
              <div
                className={`field-error${fieldErrors.height ? " visible" : ""}`}
              >
                {fieldErrors.height}
              </div>
              <div
                className={`field-warn${fieldWarnings.height ? " visible" : ""}`}
              >
            ...
            <div className="onboarding-field">
              ...
              <div
                className={`field-error${fieldErrors.weight ? " visible" : ""}`}
              >
                {fieldErrors.weight}
              </div>
              <div
                className={`field-warn${fieldWarnings.weight ? " visible" : ""}`}
              >
            ...
            <div className="onboarding-field">
              ...
              <div
                className={`field-error${fieldErrors.age ? " visible" : ""}`}
              >
                {fieldErrors.age}
              </div>
              <div
                className={`field-warn${fieldWarnings.age ? " visible" : ""}`}
              >
            ...
            <div className="onboarding-field">
              ...
              <div
                className={`field-error${fieldErrors.goal ? " visible" : ""}`}
              >
                {fieldErrors.goal}
              </div>
            </div>
            ...
            <div className="onboarding-field">
              ...
              <div
                className={`field-warn${fieldWarnings.bmi ? " visible" : ""}`}
              >
